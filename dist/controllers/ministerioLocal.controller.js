...
const ministerioLocalService = __importStar(require("../services/ministerioLocal.service"));
...
const get = async (req, res) => {
    try {
        ...
        if (!ministerio)
            return res.status(404).json({ error: 'Ministério local não encontrado.' });
        res.json(ministerio);
    }
    catch (error) {
        res.status(400).json({ error: error.message });
...
//# sourceMappingURL=ministerioLocal.controller.js.map